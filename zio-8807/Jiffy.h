 ...
 private:
  ...
  class bufferList {
   public:
    ...
    bufferList(unsigned int bufferSizez) : next(NULL), prev(NULL), head(0), positionInQueue(1) {
  ...
  bufferList* headOfQueue;  // the first array that contains data for the thread that takes elements from the queue (for
                            // the single consumer)
  std::atomic<bufferList*> tailOfQueue alignas(
      CACHE_LINE_SIZE);  // the beginning of the last array which we insert elements into (for the producers)
  unsigned int bufferSize;
  std::atomic<uint_fast64_t> gTail alignas(
      CACHE_LINE_SIZE);  // we need a global tail so the queue will be wait free - if not the queue is look free

 public:
  MpScQueue(unsigned int size) : bufferSize(NODE_SIZE), tailOfQueue(NULL), gTail(0) {
  ...
  bool dequeue(T& data)  // to take out an element - just one thread call this func
  {
    while (true) {
      ...
      if ((headOfQueue == tailOfQueue.load(std::memory_order_acquire)) &&
          (headOfQueue->head == (gTail.load(std::memory_order_acquire) - prevSize)))  // the queue is empty
      {
        // empty
        return false;
      } else if (headOfQueue->head < bufferSize)  // there is elements in the current array.
      {
        ...
        while (n->is_set.load(std::memory_order_acquire) ==
               0)  // is not set yet - try to take out set elements that are next in line
        {
          if (tempHead < bufferSize)  // there is elements in the current array.
          {
            Node* tn = &(tempHeadOfQueue->currbuffer[tempHead++]);
            if (tn->is_set.load(std::memory_order_acquire) == 1 &&
                n->is_set.load(std::memory_order_acquire) ==
                    0)  // the data is valid and the element in the head is still in insert process
            {
              //************* scan****************************
              bufferList* scanHeadOfQueue = headOfQueue;
              for (unsigned int scanHead = scanHeadOfQueue->head;
                   (scanHeadOfQueue != tempHeadOfQueue ||
                    scanHead < (tempHead - 1) && n->is_set.load(std::memory_order_acquire) == 0);
                   scanHead++) {
                ...
                if (scanN->is_set.load(std::memory_order_acquire) ==
              ...
              if (flag_moveToNewBuffer &&
                  (tempHead - 1) == tempHeadOfQueue->head)  // if we moved to a new buffer ,we need to move forward the
                                                            // head so in the end we can delete the buffer
              {
          ...
          if (tempHead >= bufferSize)  // we reach the end of the buffer -move to the next
          {
            if (flag_buffer_all_handeld &&
                flag_moveToNewBuffer)  // we want to "fold" the queue - if there is a thread that got stuck - we want to
                                       // keep only that buffer and delete the rest( upfront from  it)
            {
      ...
      if (headOfQueue->head >= bufferSize)  // move to the next array and delete the prev array
      {
        if (headOfQueue == tailOfQueue.load(std::memory_order_acquire))
          return false;  // there is no place ro move
                         // delete[] headOfArray->currbuffer; // we finished reading the current array - we can delete
                         // it and move to the next one 	headOfArray->currbuffer = NULL;
        bufferList* next = headOfQueue->next.load(std::memory_order_acquire);
        if (next == NULL) return false;  // if we do not have where to move
        my_align_free(headOfQueue);
        headOfQueue = next;
  ...
  void enqueue(T const& data)  // to put in an element
  {
    ...
    while (true) {
      ...
      unsigned int prevSize =
          bufferSize * (tempTail->positionInQueue - 1);  // the amount of items in the queue without the current buffer

      while (location <
             prevSize)  // the location is back in the queue - we need to go back in the queue to the right buffer
      {
        go_back = true;
        ...
        if (next == NULL)  // we do not have a next buffer - so we can try to allocate a new one
        {
          ...
          if ((tempTail->next)
                  .compare_exchange_strong(Nullptr,
                                           newArr))  //	if (CAS(tailOfQueue->next, NULL, newArr)) 1 next-> new array
          {
